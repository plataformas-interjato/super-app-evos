  ...
  const takePhoto = async (entryId: number) => {
    ...
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        console.log('📸 Foto de coleta capturada, iniciando compressão...');
        ...
        try {
          ...
          console.log(`✅ Foto de coleta comprimida: ${compressed.compressionRatio.toFixed(1)}% redução (${(compressed.originalSize/(1024*1024)).toFixed(2)}MB → ${(compressed.compressedSize/(1024*1024)).toFixed(2)}MB)`);
          
          photoUriToSave = compressed.uri;
          
        } catch (compressionError) {
          console.warn('⚠️ Erro na compressão da foto de coleta, usando original:', compressionError);
        }
        
        console.log('💾 Salvando foto no sistema unificado (FileSystem)...');
        
        // Salvar usando sistema unificado (FileSystem) ao invés de AsyncStorage
        try {
          ...
          if (result.success) {
            ...
            // Atualizar estado local para exibição
            setCollectedPhotos(prev => ({
              ...prev,
              [entryId]: photoUriToSave
            }));
            
            console.log(`✅ Foto salva - entrada ${entryId}`);
          } else {
            console.error('❌ Erro ao salvar no sistema unificado:', result.error);
  ...
  const takePhotoFromModal = async () => {
    try {
      ...
      try {
        ...
        if (!result.canceled && result.assets[0]) {
          const originalUri = result.assets[0].uri;
          console.log('📸 Foto de coleta (modal) capturada, iniciando compressão...');
          ...
          try {
            ...
            console.log(`✅ Foto de coleta (modal) comprimida: ${compressed.compressionRatio.toFixed(1)}% redução`);
            
            photoUriToSave = compressed.uri;
            
          } catch (compressionError) {
            console.warn('⚠️ Erro na compressão da foto de coleta (modal), usando original:', compressionError);
          }
          ...
          try {
            ...
            if (result.success) {
              ...
              console.log(`✅ Foto salva - entrada ${selectedEntryForModel.id}`);
            } else {
              console.error('❌ Erro ao salvar no sistema unificado:', result.error);
              Alert.alert('Erro', 'Não foi possível salvar a foto. Tente novamente.');
            }
